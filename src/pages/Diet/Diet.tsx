...
export default function Diet() {
  ...
  const [applyCalories, setApplyCalories] = useState(0);
  ...
  const handleApplyCalories = () => {
    if (applyCalories <= 0 || !applyLabel.trim()) return;
    const entry: CalorieLogEntry = {
      label: applyLabel.trim(),
      calories: applyCalories,
  ...
  const inputStyle = { ...theme.input, width: 95 };
          ...
          <>
            ...
            <div style={{ display: "flex", gap: 16, alignItems: "flex-end" }}>
              ...
              <div style={{ position: "relative", alignSelf: "flex-end" }}>
                <Button onClick={() => setShowApplyPopup(true)} size="md">
                  Add Item
                </Button>
                {showApplyPopup && (
                  <>
                  <div onClick={() => { setShowApplyPopup(false); setApplyLabel(""); setApplyCalories(0); }} style={{ position: "fixed", inset: 0, zIndex: 10 }} />
                  <div onClick={(e) => e.stopPropagation()} style={{
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (!isNaN(n) && n <= 10000) setApplyCalories(n);
                      }}
                      onKeyDown={(e) => { if (e.key === "Enter" && applyLabel.trim() && applyCalories > 0) handleApplyCalories(); }}
                      placeholder="500"
                      style={inputStyle}
                    />
                    <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
                      <Button onClick={() => { setShowApplyPopup(false); setApplyLabel(""); setApplyCalories(0); }} size="sm" variant="secondary">
                        Cancel
                      </Button>
                      <Button onClick={handleApplyCalories} size="sm" disabled={!applyLabel.trim() || applyCalories <= 0}>
                        Save
                      </Button>
                    </div>
                  </div>
